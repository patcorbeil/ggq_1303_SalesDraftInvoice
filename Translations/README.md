...
1. Rebuild the app (Ctrl+Shift+B). With the "TranslationFile" feature enabled, the compiler now generates `Translations\Draft Invoice Currency.g.xlf` (source language, all labels).
2. Copy that file and rename the copy to `Draft Invoice Currency.fr-CA.xlf` (keep it in the Translations folder).
3. Edit the copy:
